    ...
    <div class="data-card">
        ...
        <div class="control-header">
            ...
            <div class="d-flex" style="gap: 12px;">
                ...
                <a href="<%= request.getContextPath() %>/EmployeeServlet?action=new" class="btn btn-primary-flat">
        ...
        <div class="d-flex mb-4 align-items-center justify-content-end">
            <form action="EmployeeServlet" method="GET" class="search-container">
                ...
                <!-- 搜索输入框 -->
                <input type="text" name="search" class="form-control" placeholder="按姓名、部门或职位搜索..." value="${requestScope.searchKeyword}">
            ...
            <table class="table table-minimal">
                ...
                <tr>
                    <th scope="col" style="width: 5%;">
                        <a href="EmployeeServlet?action=list&search=${requestScope.searchKeyword}&page=${requestScope.currentPage}&sortBy=id&sortOrder=<c:choose><c:when test="${requestScope.sortBy eq 'id' and requestScope.sortOrder eq 'ASC'}">DESC</c:when><c:otherwise>ASC</c:otherwise></c:choose>" class="sort-link">
                            ID
                    ...
                    <th scope="col" style="width: 15%;">
                        <a href="EmployeeServlet?action=list&search=${requestScope.searchKeyword}&page=${requestScope.currentPage}&sortBy=name&sortOrder=<c:choose><c:when test="${requestScope.sortBy eq 'name' and requestScope.sortOrder eq 'ASC'}">DESC</c:when><c:otherwise>ASC</c:otherwise></c:choose>" class="sort-link">
                            姓名
                    ...
                    <th scope="col" style="width: 15%;">
                        <a href="EmployeeServlet?action=list&search=${requestScope.searchKeyword}&page=${requestScope.currentPage}&sortBy=salary&sortOrder=<c:choose><c:when test="${requestScope.sortBy eq 'salary' and requestScope.sortOrder eq 'ASC'}">DESC</c:when><c:otherwise>ASC</c:otherwise></c:choose>" class="sort-link">
                            薪资
                    ...
                    <th scope="col" style="width: 15%;">
                        <a href="EmployeeServlet?action=list&search=${requestScope.searchKeyword}&page=${requestScope.currentPage}&sortBy=hire_date&sortOrder=<c:choose><c:when test="${requestScope.sortBy eq 'hire_date' and requestScope.sortOrder eq 'ASC'}">DESC</c:when><c:otherwise>ASC</c:otherwise></c:choose>" class="sort-link">
                            入职日期
                ...
                <c:forEach var="employee" items="${requestScope.listEmployee}">
                    <tr>
                        ...
                        <td class="text-muted"><c:out value="${employee.hireDate}" /></td>
                        <td class="text-center">
                            <!-- 扁平化操作链接 -->
                            <a href="<%= request.getContextPath() %>/EmployeeServlet?action=view&id=<c:out value='${employee.id}' />" class="action-link">
                                <i class="fas fa-eye me-1"></i> 查看
                            </a>
                            <a href="<%= request.getContextPath() %>/EmployeeServlet?action=edit&id=<c:out value='${employee.id}' />" class="action-link">
                                <i class="fas fa-edit me-1"></i> 编辑
                            </a>
                            <a href="<%= request.getContextPath() %>/EmployeeServlet?action=delete&id=<c:out value='${employee.id}' />" class="action-link delete" onclick="return confirm('确定删除员工 ${employee.name} 吗？');">
                ...
                <c:if test="${empty requestScope.listEmployee}">
                    <tr>
        ...
        <nav aria-label="Page navigation" class="mt-5 pt-3 border-top" style="border-color: var(--border-color) !important;">
            <ul class="pagination justify-content-center">
                ...
                <li class="page-item <c:if test="${requestScope.currentPage == 1}">disabled</c:if>">
                    <a class="page-link" href="EmployeeServlet?action=list&search=${requestScope.searchKeyword}&page=${prevPage}&sortBy=${requestScope.sortBy}&sortOrder=${requestScope.sortOrder}"><i class="fas fa-chevron-left"></i></a>
                </li>

                <!-- 页码列表 -->
                <c:forEach begin="1" end="${requestScope.totalPages}" var="i">
                    <li class="page-item <c:if test="${requestScope.currentPage == i}">active</c:if>">
                        <a class="page-link" href="EmployeeServlet?action=list&search=${requestScope.searchKeyword}&page=${i}&sortBy=${requestScope.sortBy}&sortOrder=${requestScope.sortOrder}"><c:out value="${i}"/></a>
                    </li>
                </c:forEach>

                <!-- 下一页按钮 -->
                <li class="page-item <c:if test="${requestScope.currentPage == requestScope.totalPages}">disabled</c:if>">
                    <a class="page-link" href="EmployeeServlet?action=list&search=${requestScope.searchKeyword}&page=${nextPage}&sortBy=${requestScope.sortBy}&sortOrder=${requestScope.sortOrder}"><i class="fas fa-chevron-right"></i></a>
                </li>
            </ul>
        </nav>

        <p class="text-center text-muted mt-3">
            <small>当前显示 ${requestScope.listEmployee.size()} 条记录 / 总记录数 ${requestScope.totalRecords} 条，共 ${requestScope.totalPages} 页</small>
        </p>
    </div>
</div>
<!-- 引入 Bootstrap JS (用于关闭 Alert) -->
